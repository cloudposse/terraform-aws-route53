...
| <a name="input_allow_overwrite"></a> [allow\_overwrite](#input\_allow\_overwrite) | Allow overwriting existing records | `bool` | `false` | no |
...
| <a name="input_default_ttl"></a> [default\_ttl](#input\_default\_ttl) | Default Time To Live (TTL) for records, when not explicitly defined in the records set | `number` | `3600` | no |
...
| <a name="input_force_destroy"></a> [force\_destroy](#input\_force\_destroy) | Set to true if you want to remove all records in the zone. Not recommended as some records may be created outside the terraform (e.g. such as from the `external-dns` controller) | `bool` | `false` | no |
